...
「2026-07-26 変更: 旧→新」「（2026-07-13 追加）」のような**過程**は
`docs/devlog.md` の担当。仕様書に置くと次の3つが起きる:

  1. 読み手が「今どっちなのか」を判断するために履歴を読まされる
     （見出しにまで混ざると特に読みにくい）
  2. 同じ経緯が devlog と仕様書の2箇所に散る（片方だけ直ると矛盾する）
  3. 古い決定が消えないまま積もり、仕様書が更新履歴のログになる

検査（`docs/specification/**/*.md` が対象。devlog と design-notes は過程を書く場所なので
対象外。
...
「いつ何から何へ変えたか」という**時系列**。理由を残しつつ経緯を落とす書き換え例:
  ...
  NG: 旧実装は A > B だったため取りこぼしていた（2026-07-26 変更）
  OK: 「深刻さ」ではなく「ユーザーの番か」で並べる（取りこぼさないため）
...
# 「以前は」は「それ以前は」（時刻表示の説明）と紛れるので指示語を除外する。
HISTORY_WORDS = ("旧実装", "従来は", r"(?<![それこ])以前は", "当初は", "旧仕様",
                 "変更前は", "だった経緯")
...
# （`<time datetime="2025-12-10...">` やファイル名の日付）まで履歴と判定すると、
...
def test_spec_no_history() -> None:
    """pytest から収集されたときも同じ検査を行う（SystemExit は __main__ だけ）。"""
    assert not failures(), "\n".join(failures())
