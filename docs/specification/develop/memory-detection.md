...
- **フッタが提案するコマンドは「いま見ている並び順を再現するコマンド」にする**
  （`COMMAND_BY_SORT[order]`）。起動に使ったコマンド名をそのまま出すと、
  `memhog --sort cpu` で見ているのに `memhog --app X` を勧めることになり、
  開き直した画面の並びが変わる。表示は成立し実行もできる（並びが違うだけ）ので、
  指摘されるまで気づけない。
